...
**在线 AI 助教**：双击 `learn/启动学习助教.cmd`，点击角色 → API 设置，输入你自己的服务地址、模型和 Key。Key 在仓库外用 Windows 用户 DPAPI 加密保存。详见 [助教使用说明](AI-ASSISTANT.md)。第一次从离线地址切换到本机 HTTP 地址，请先导出再恢复旧进度。
...
用户笔记不写入源码或 Git，备份文件 `blaster-progress-*.json` 在 learn 内永久忽略。离线内容始终可读；通过本机启动器启用的 AI 问答/评价记录也包含在学习备份内，API Key 和设置从不包含在备份中。
