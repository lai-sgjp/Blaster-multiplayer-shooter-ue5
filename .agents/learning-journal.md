...
本文件保存迁移前已经积累的历史知识。新的学习内容按主题写入项目根目录 `learn/`，例如动画写入 `learn/animation/`，网络写入 `learn/networking/`，C++ 写入 `learn/cpp/`，Editor 操作写入 `learn/editor/`，故障排查写入 `learn/debugging/`。这里保留原有章节，避免破坏历史记录；新增内容必要时只添加索引链接。
