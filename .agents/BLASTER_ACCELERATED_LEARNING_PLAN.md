...
> 2026-09-10 归档定位：本文件保留为 **B00–B12 工程工作包与课程覆盖参考**。当前学习顺序与节奏统一采用 [面试学习路线](../learn/INTERVIEW-LEARNING-ROADMAP.md)，学习和记录入口为 [Blaster Lab](../learn/index.html)。下方 Week 1–4 是原工程时间盒，不是第二套需要重复执行的学习日程。独有的预测、其他 SSR、Teams/CTF 等进入选修设计；完成状态以当前源码及验证矩阵为准。[合并说明](../learn/site/PLAN-RECONCILIATION.md)。
...
- 初版制定时工程大致在课程第 4 节；此描述现已过时，不再作为功能现状。当前源码覆盖核心战斗、比赛、HUD 与 Hitscan 回溯，FABRIK 资产和发布网络等边界以 `docs/VERIFICATION.md` 为准。
...
5. **实现与教学**：先解释概念、源码入口、数据流和验证，再邀请用户复述；不会作答不阻止继续教学。工程实现仍按最小切片推进，不进行无关重构。
